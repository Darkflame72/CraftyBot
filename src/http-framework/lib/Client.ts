...
export class Client {
  ...
  public constructor(options: ClientOptions = {}) {
    const discordPublicKey =
      options.discordPublicKey ?? process.env.DISCORD_PUBLIC_KEY;
    if (!discordPublicKey)
      throw new Error("The discordPublicKey cannot be empty");
    ...
    switch (interaction.type) {
      case InteractionType.ApplicationCommand:
        return container.stores
          .get("commands")
          .runApplicationCommand(reply, interaction);
      case InteractionType.ApplicationCommandAutocomplete:
        return container.stores
          .get("commands")
          .runApplicationCommandAutoComplete(reply, interaction);
      case InteractionType.MessageComponent:
        return container.stores
          .get("message-component-handlers")
          .runHandler(reply, interaction);
      default:
        return reply
          .status(HttpCodes.NotImplemented)
          .send({ message: "Unknown interaction type" });
    }
  }

  protected verifyDiscordInteraction(
    request: FastifyRequest
  ): VerifyDiscordInteractionResponse | null {
    const { headers } = request;
    const signatureHeader = headers["x-signature-ed25519"] as
      | string
      | undefined;
    const timestampHeader = headers["x-signature-timestamp"] as
      | string
      | undefined;
