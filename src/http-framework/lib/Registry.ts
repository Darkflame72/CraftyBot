...
export class Registry {
  ...
  public get allCommands(): RESTPostAPIApplicationCommandsJSONBody[] {
    return [
      ...chatInputCommandRegistry.values(),
      ...flattenIterableOfArrays(contextMenuCommandRegistry.values()),
    ];
  }
  ...
  public get globalCommands(): RESTPostAPIApplicationCommandsJSONBody[] {
    return [
      ...chatInputCommandRegistry
        .filter((_, command) => !restrictedGuildIdRegistry.has(command))
        .values(),
      ...flattenIterableOfArrays(
        contextMenuCommandRegistry
          .filter((_, command) => !restrictedGuildIdRegistry.has(command))
  ...
  public get guildCommands(): Collection<
    string,
  ...
  public registerGlobalCommands() {
    let body: RESTPutAPIApplicationCommandsJSONBody = this.globalCommands;
    // remove duplicate commands in body
    body = body.filter(
      (command, index, self) =>
   ...
   */
  public registerGuildRestrictedCommands(): Promise<
    PromiseSettledResult<RESTPutAPIApplicationGuildCommandsResult>[]
  > {
...
export interface RegistryOptions
  extends Partial<RESTOptions>,
    Pick<RequestData, "authPrefix"> {
  /**
